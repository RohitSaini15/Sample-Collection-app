...
module.exports.getAllSamples=async(req,res)=>{
  try {
      ...
      res.status(200).send({samples,status: "SUCCESS"});
  } catch (error) {
      console.log(`error occured in sample_controller approvedSamples ${err}`);
...
module.exports.afterSignature = async (req, res) => {
  try {
    ...
    return res.status(401).json({ msg: "error occured in saving the signture" ,status:"ERROR"});
  }
};
...
module.exports.dataForPdf = async (req, res) => {
  try {
    ...
    let signature = await Signature.find({sample:req.params.id});
    if (!signature) {
      res.send({msg:"signature does not exists",status:"ERROR"});
    }
    let result = {
      city: sample.city,
      name_of_fso: sample.name_of_fso,
    ...
    console.log(`error occured in sample_controller dataForPdf ${err}`);
    return res.status(401).json({ msg: "error occured while getting data for pdf" ,status:"ERROR"});
  }
